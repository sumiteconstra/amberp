        ...
        const transformedData = res.data.map((item, index) => ({
          ...
          status_return:
            item.status === 6 && item.uploadpo == null
              ? `<label class="badge badge-outline-meantGreen mb-0"><i class="fas fa-circle f-s-8 d-flex me-1"></i>Followup Done</label>`
              : item.status === 6 &&
                item.uploadpo != null &&
                item.productsreprodut.length == 0
                ? `<label class="badge badge-outline-success"><i class="fas fa-circle f-s-8 d-flex me-1"></i>PO Uploaded</label>`
                : item.status === 6 &&
                  item.uploadpo != null &&
                  item.productsreprodut.length !== 0
                  ? `<label class="badge badge-outline-warning "><i class="fas fa-circle f-s-8 d-flex me-1"></i>Revised Product</label>`
                  : "Unknown",
        }));
...
export default MysalesOrderListDone;
