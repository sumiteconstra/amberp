...
function PurchaseOrderRecv() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        getBillNumber(data.reference_number + '/' + Math.floor(Math.random() * 1000));
        setBuyer(data.buyer);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await PrivateAxios.post(`purchase/recv/new/${id}`, data);
                          ...
                          <tbody>
                            {products.map((product, index) => (
                              <tr key={index}>
                                ...
                                <td><div className="min-width-100">{Number(product.taxExcl).toFixed(2) || 0}</div></td>
                              </tr>
                            ))}
                          </tbody>
                        </Table>
